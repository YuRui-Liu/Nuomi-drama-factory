...
const m = vi.hoisted(() => ({ mutate: vi.fn(), start: vi.fn(), refetch: vi.fn() }));
...
vi.mock("@/lib/queries/narrative-groups",()=>({
 useNarrativeGroups:()=>({data:{ok:true,data:[group]},isLoading:false,refetch:vi.fn()}),
...
describe("NarrativeGroupWorkbench references",()=>{
 beforeEach(()=>{vi.clearAllMocks();m.mutate.mockResolvedValue({scope:"x"});});
