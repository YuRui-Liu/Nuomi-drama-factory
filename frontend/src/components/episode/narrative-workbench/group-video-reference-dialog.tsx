...
function validate(items: VideoReferenceSelection[], minImages: number, maxImages: number, t: TFunction): string | null {
  if (items.length < minImages) return t("narrativeVideoReferences.validation.min", { min: minImages });
  if (items.length > maxImages) return t("narrativeVideoReferences.validation.max", { max: maxImages });
  if (new Set(items.map((item) => item.reference_id)).size !== items.length) return t("narrativeVideoReferences.validation.duplicate");
  for (const item of items) {
    const description = item.subject_description.trim();
    if (!description) return t("narrativeVideoReferences.validation.empty");
    if (/\r|\n/.test(description)) return t("narrativeVideoReferences.validation.singleLine");
    if (description.length > 500) return t("narrativeVideoReferences.validation.tooLong");
  }
  return null;
}
...
export function GroupVideoReferenceDialog({
  ...
  const validationError = validate(draft, minImages, maxImages, t);
  ...
  const handleUpload = async (file?: File) => {
    ...
    try {
      ...
      if (!uploaded) throw new Error(t("narrativeVideoReferences.uploadFailed"));
      ...
      setSubmitError(cause instanceof Error ? cause.message : t("narrativeVideoReferences.uploadFailed"));
    }
  };
  const handleSave = async () => {
    ...
    try {
      ...
      if (!saved) throw new Error(t("narrativeVideoReferences.saveFailed"));
      onSaved?.(saved);
      onOpenChange(false);
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : t("narrativeVideoReferences.saveFailed");
      setSubmitError(/409|revision|conflict/i.test(message)
        ? t("narrativeVideoReferences.conflict")
        : message);
    }
  };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl" aria-describedby={undefined}>
      <DialogHeader><DialogTitle>{t("narrativeVideoReferences.title")}</DialogTitle></DialogHeader>
      {loading ? <p role="status" className="text-sm text-muted-foreground">{t("narrativeVideoReferences.loading")}</p> : null}
      {error ? <p role="alert" className="text-sm text-destructive">{t("narrativeVideoReferences.loadFailed")}</p> : null}
      {!loading && !error && preview ? <>
        <section>
          <div className="flex items-center justify-between gap-2"><h3 className="font-medium">{t("narrativeVideoReferences.candidates")}</h3><label className="cursor-pointer text-xs text-primary"><Upload className="mr-1 inline size-3" />{t("narrativeVideoReferences.upload")}<input className="sr-only" aria-label={t("narrativeVideoReferences.upload")} type="file" accept="image/*" disabled={upload.isPending} onChange={(event) => void handleUpload(event.target.files?.[0])} /></label></div>
          <div className="mt-2 grid gap-2 sm:grid-cols-3">{localCandidates.map((candidate) => <button key={candidate.reference_id} type="button" className="flex items-center gap-2 rounded border border-white/10 p-2 text-left disabled:opacity-50" disabled={draft.length >= maxImages || draft.some((item) => item.reference_id === candidate.reference_id)} onClick={() => add(candidate)}>
            ...
            <span><span className="block text-xs font-medium">{candidate.label}</span><span className="text-[11px] text-muted-foreground">{t(`narrativeVideoReferences.source.${candidate.source_kind}`)}</span></span>
          </button>)}</div>
        </section>
        <section><h3 className="font-medium">{t("narrativeVideoReferences.selectedCount", { count: draft.length, max: maxImages })}</h3><div className="mt-2 space-y-2">{draft.map((item, index) => {
          const candidate = candidateById.get(item.reference_id);
          const label = candidate?.label ?? item.reference_id;
          return <div key={item.reference_id} data-testid={`selected-${item.reference_id}`} draggable onDragStart={() => { dragIndex.current = index; }} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragIndex.current !== null) move(dragIndex.current, index); dragIndex.current = null; }} className="grid gap-2 rounded border border-white/10 p-2 sm:grid-cols-[auto_1fr_auto]">
            ...
            <label className="text-xs"><span className="font-medium">{t("narrativeVideoReferences.picture", { index: index + 1 })}</span><span className="ml-2 text-muted-foreground">{t("narrativeVideoReferences.subject", { index: index + 1 })} · {t(`narrativeVideoReferences.source.${candidate?.source_kind ?? "temporary_upload"}`)} · {label}</span><textarea rows={1} className="mt-1 min-h-9 w-full resize-y rounded border border-input bg-background px-2 py-2" value={item.subject_description} aria-label={t("narrativeVideoReferences.description", { label })} onChange={(event) => setDraft((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, subject_description: event.target.value } : entry))} /></label>
            <div className="flex gap-1"><Button type="button" size="icon-sm" variant="ghost" aria-label={t("narrativeVideoReferences.moveUp", { label })} disabled={index === 0} onClick={() => move(index, index - 1)}><ArrowUp /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label={t("narrativeVideoReferences.moveDown", { label })} disabled={index === draft.length - 1} onClick={() => move(index, index + 1)}><ArrowDown /></Button><Button type="button" size="sm" variant="ghost" onClick={() => setDraft((current) => current.filter((_, entryIndex) => entryIndex !== index))}>{t("narrativeVideoReferences.remove")}</Button></div>
          </div>;
        })}</div></section>
      ...
      <DialogFooter className="p-0"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>{t("narrativeVideoReferences.cancel")}</Button><Button type="button" disabled={!preview || !!validationError || update.isPending || upload.isPending} onClick={() => void handleSave()}>{update.isPending ? t("narrativeVideoReferences.saving") : t("narrativeVideoReferences.save")}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
