...
export function NarrativeGroupWorkbench({ project, episode, onRepairBeat }: { project: string; episode: number; onRepairBeat: (beatId: string) => void }) {
  ...
  const runGroupVideo = async (request: { video_model: string; h3_mode: "auto" | "i2va" | "fl2va" }) => {
    const currentReferencePreview = savedReferencePreview?.groupId === group.id
      ? savedReferencePreview.preview : queriedReferencePreview;
    if (referencePolicy && (videoReferenceDirty || !validVideoReferences(currentReferencePreview, referencePolicy))) return;
    try {
  ...
  const referencePreview = savedReferencePreview?.groupId === group.id
    ? savedReferencePreview.preview : queriedReferencePreview;
  const selectedReferences = referencePreview?.selected ?? [];
  const referenceValid = validVideoReferences(referencePreview, referencePolicy);
