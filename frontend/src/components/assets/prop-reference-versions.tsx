...
interface PropReferenceVersionsProps {
  project: string;
  propName: string;
...
export function PropReferenceVersions({
  project,
  propName,
  legacyAssetPath,
}: PropReferenceVersionsProps) {
  const slotId = `prop:${propName}:reference`;
  ...
  const adopt = async (versionId: string) => {
    try {
      await adoptVersion.mutateAsync({ versionId, reason: "道具参考图手动采用" });
      toast.success("已采用道具参考图版本");
    } catch {
  ...
  return (
    <section className="rounded-[10px] border border-border bg-white/[0.02] p-3" aria-label="道具参考图版本">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h4 className="text-xs font-medium text-foreground">道具参考图版本</h4>
          <p className="mt-1 text-[11px] text-muted-foreground">
            正面、严格侧面、背面保持同一道具；候选通过 QC 后可手动采用。
          </p>
        </div>
          ...
          return (
            <article key={version.version_id} className="overflow-hidden rounded-[8px] border border-border bg-background/30">
              <img
                ...
                alt={`${propName} 道具参考图 ${version.version_id}`}
              ...
              <div className="space-y-2 p-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {panelLayout(version).map((panel) => (
                    <span key={panel} className="rounded bg-white/[0.05] px-1.5 py-0.5 text-[10px] text-muted-foreground">
