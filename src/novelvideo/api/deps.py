...
def get_media_capability_store():
    """Return the instance-local media capability configuration store."""
    from novelvideo import config as app_config
    from novelvideo.media_capabilities.store import MediaCapabilityStore

    return MediaCapabilityStore(Path(app_config.STATE_DIR) / "local" / "settings.db")
...
def get_production_store():
    """Return the instance-wide production DAG store."""
    from novelvideo import config as app_config
    from novelvideo.media_capabilities.production.store import ProductionStore

    return ProductionStore(Path(app_config.STATE_DIR) / "local" / "production.db")
