...
MAX_WORKFLOW_IMPORT_REQUEST_BODY_BYTES = 6 * 1024 * 1024
...
class _RequestBodyTooLarge(Exception):
    def __init__(self, *, limit: int, got: int) -> None:
        self.limit = limit
        self.got = got
...
def _request_body_limit(request: Request) -> int:
    ...
    if (
        ...
        and request.url.path == "/api/v1/media-capabilities/workflows/import"
        and "multipart/form-data" in content_type
    ):
        return MAX_WORKFLOW_IMPORT_REQUEST_BODY_BYTES
    if (
...
class _StreamingBodyLimitMiddleware:
    """Enforce request limits from received ASGI chunks, not trusted headers."""
    ...
    async def __call__(
        ...
        async def limited_receive():
            ...
            if message["type"] == "http.request":
                ...
                if received > limit:
                    exceeded = True
                    raise _RequestBodyTooLarge(limit=limit, got=received)
            return message
...
def create_app() -> FastAPI:
    ...
    application.add_middleware(_StreamingBodyLimitMiddleware)
    return application
