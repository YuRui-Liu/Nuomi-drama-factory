...
from .config import (  # noqa: F401
    ...
    init_cognee,
    resolve_cognee_project_storage_paths,
)
...
class CogneeStore:
    ...
    def __init__(
        ...
        self.knowledge_rebuild_requested = False
        self._knowledge_rebuild_prepared = False
    ...
    def embedding_model_scope(self):
        binding = getattr(self, "cognee_embedding_binding", None)
        if binding is not None:
            return embedding_spec_scope(binding)
    ...
    async def initialize(self):
        ...
        try:
            ...
            runtime = require_knowledge_runtime_context()
        except Exception as exc:
            from novelvideo.knowledge_runtime.settings import KnowledgeRuntimeError

            if not isinstance(exc, KnowledgeRuntimeError) or exc.code != (
                "KNOWLEDGE_RUNTIME_CONTEXT_MISSING"
            ):
                raise
            embedding_binding = ensure_cognee_embedding_binding_in_state_dir(
                self.state_dir
            )
        else:
            embedding_binding = prepare_cognee_embedding_binding_in_state_dir(
                self.state_dir,
                runtime.embedding,
                rebuild=bool(getattr(self, "knowledge_rebuild_requested", False)),
            )
        self.cognee_embedding_binding = embedding_binding
    ...
    async def ingest_novel_fast(
        ...
        if rebuild:
            report(0.05, "重建图谱...")
            log("清除 cognee 图谱数据...")
            if not self._knowledge_rebuild_prepared:
...
请直接回复身份编号（如 1、2、3），不要有其他内容。"""
    ...
    async def _prune_cognee_only(self):
        ...
        # setup() may already have cached an open Ladybug engine for this path.
    ...
    async def prepare_knowledge_rebuild(self) -> None:
        """Clear Cognee storage before setup opens database connections."""
        await self._prune_cognee_only()
        self._knowledge_rebuild_prepared = True
