...
from novelvideo.cognee.codex_adapter import install_codex_llm_gateway_adapter
...
from novelvideo.knowledge_runtime.settings import (
...
def _clear_cognee_embedding_and_vector_engine_caches() -> None:
    """Clear Cognee singletons that capture embedding configuration."""
    ...
    for module_name, attribute in cache_targets:
...
def _apply_ollama_embedding_env(
    ...
    endpoint = f"{settings.base_url.rstrip('/')}/api/embed"
    os.environ["EMBEDDING_PROVIDER"] = "ollama"
    os.environ["EMBEDDING_MODEL"] = settings.model
    os.environ["EMBEDDING_DIMENSIONS"] = str(settings.dimension)
    os.environ["EMBEDDING_BATCH_SIZE"] = str(settings.batch_size)
    os.environ["EMBEDDING_ENDPOINT"] = endpoint
    normalized_model = settings.model.strip().casefold()
    if normalized_model.startswith("qwen3-embedding"):
        huggingface_tokenizer = "Qwen/Qwen3-Embedding-0.6B"
    elif normalized_model.startswith("bge-m3"):
    ...
    return "ollama", settings.model, str(settings.dimension), ""
...
def resolve_cognee_project_storage_paths(
    state_dir: str | os.PathLike[str],
) -> tuple[Path, Path]:
    state_path = Path(state_dir).resolve()
    configured_root = os.environ.get("NOVELVIDEO_COGNEE_ROOT", "").strip()
    if configured_root:
        project_key = hashlib.sha256(
            os.path.normcase(str(state_path)).encode("utf-8")
        ).hexdigest()[:20]
        state_path = Path(configured_root).expanduser().resolve() / project_key
    return state_path / "cognee_system", state_path / "cognee_data"
...
def apply_cognee_project_storage_context(
    ...
    system_path, data_path = resolve_cognee_project_storage_paths(state_dir)
    cognee_system_dir = str(system_path)
    cognee_data_dir = str(data_path)
