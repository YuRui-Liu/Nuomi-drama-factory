...
DEFAULT_MEDIA_PIPELINE = "legacy"
PROJECT_EMBEDDING_BINDING_KEY = "cognee_embedding_binding"


@dataclass(frozen=True)
class CogneeEmbeddingBindingStatus:
    ready: bool
    rebuild_required: bool
    reason: str
    binding: "EmbeddingModelSpec | None"


@dataclass(frozen=True)
class MediaPipelineSettings:
    """Project rollout switches for the production media pipeline."""

    image_pipeline: Literal["legacy", "production"] = DEFAULT_MEDIA_PIPELINE
    video_pipeline: Literal["legacy", "production"] = DEFAULT_MEDIA_PIPELINE
    tts_pipeline: Literal["legacy", "production"] = DEFAULT_MEDIA_PIPELINE
    production_scheduler: bool = False

    @classmethod
    def from_config(cls, config: dict | None) -> "MediaPipelineSettings":
        raw = config or {}

        def pipeline_value(key: str) -> Literal["legacy", "production"]:
            value = raw.get(key)
            return value if value in {"legacy", "production"} else "legacy"

        return cls(
            image_pipeline=pipeline_value("image_pipeline"),
            video_pipeline=pipeline_value("video_pipeline"),
            tts_pipeline=pipeline_value("tts_pipeline"),
            production_scheduler=raw.get("production_scheduler") is True,
        )


def resolve_media_pipeline(
    settings: MediaPipelineSettings,
    media_type: Literal["image", "video", "tts"],
    *,
    production_available: bool = True,
) -> Literal["legacy", "production"]:
    """Resolve the guarded pipeline, falling back to the legacy implementation."""

    if media_type not in {"image", "video", "tts"}:
        raise ValueError(f"unsupported media pipeline: {media_type}")
    selected = getattr(settings, f"{media_type}_pipeline")
    if (
        settings.production_scheduler
        and production_available
        and selected == "production"
    ):
        return "production"
    return "legacy"
...
def _ollama_spec_from_payload(payload: object) -> "EmbeddingModelSpec":
    ...
    if not isinstance(payload, dict) or payload.get("provider") != "ollama":
        raise RuntimeError("Invalid project Ollama embedding binding")
    try:
        return EmbeddingModelSpec.ollama(
            model=str(payload.get("model") or ""),
            dimensions=int(payload.get("dimension") or 0),
            digest=str(payload.get("digest") or ""),
            generation=int(payload.get("generation") or 0),
        )
    except (TypeError, ValueError) as exc:
        raise RuntimeError("Invalid project Ollama embedding binding") from exc


def inspect_cognee_embedding_binding_in_state_dir(
    state_dir: str | Path,
    current: object,
) -> CogneeEmbeddingBindingStatus:
    from novelvideo.embedding_models import EmbeddingModelSpec

    state_path = Path(state_dir)
    config = load_project_config_file_from_state_dir(state_path)
    expected = EmbeddingModelSpec.ollama(
        model=str(getattr(current, "model", "")),
        dimensions=int(getattr(current, "dimension", 0)),
        digest=str(getattr(current, "digest", "")),
        generation=2,
    )
    payload = config.get(PROJECT_EMBEDDING_BINDING_KEY)
    if payload is not None:
        binding = _ollama_spec_from_payload(payload)
        if binding.fingerprint == expected.fingerprint:
            return CogneeEmbeddingBindingStatus(True, False, "ready", binding)
        return CogneeEmbeddingBindingStatus(
            False, True, "ollama_binding_mismatch", binding
        )
    if "cognee_embedding_model" in config or "cognee_embedding_dimension" in config:
        return CogneeEmbeddingBindingStatus(
            False, True, "legacy_newapi_binding", None
        )
    if (state_path / "cognee_system").exists():
        return CogneeEmbeddingBindingStatus(
            False, True, "legacy_unbound_index", None
        )
    return CogneeEmbeddingBindingStatus(False, False, "unbound", None)


def commit_cognee_embedding_binding_in_state_dir(
    state_dir: str | Path,
    binding: "EmbeddingModelSpec",
) -> None:
    if binding.provider != "ollama":
        raise ValueError("only Ollama bindings can be committed")
    config_path = get_project_config_path_from_state_dir(state_dir)
    with _project_config_lock(config_path):
        config = load_project_config_file_from_path(config_path)
        config[PROJECT_EMBEDDING_BINDING_KEY] = {
        ...
        config.pop("cognee_embedding_model", None)
        config.pop("cognee_embedding_dimension", None)
        _write_project_config_atomic(config_path, config)


def prepare_cognee_embedding_binding_in_state_dir(
    state_dir: str | Path,
    current: object,
    *,
    rebuild: bool,
) -> "EmbeddingModelSpec":
    """Select the Ollama vector space without silently mixing old indexes."""
    ...
    status = inspect_cognee_embedding_binding_in_state_dir(state_dir, current)
    if status.ready:
        return binding
    if status.rebuild_required and not rebuild:
        raise KnowledgeRuntimeError(
            "当前项目的 Cognee 索引来自旧嵌入模型或其他 Ollama 模型；请明确选择重建知识索引。",
            code="KNOWLEDGE_INDEX_REBUILD_REQUIRED",
        )
    if not status.rebuild_required:
        commit_cognee_embedding_binding_in_state_dir(state_dir, binding)
    return binding
...
def ensure_cognee_embedding_binding_in_state_dir(
    ...
    with _project_config_lock(config_path):
        ...
        ollama_payload = config.get(PROJECT_EMBEDDING_BINDING_KEY)
        if ollama_payload is not None:
            return _ollama_spec_from_payload(ollama_payload)
...
def _default_project_config() -> dict:
    ...
    return {
        ...
        "image_pipeline": DEFAULT_MEDIA_PIPELINE,
        "video_pipeline": DEFAULT_MEDIA_PIPELINE,
        "tts_pipeline": DEFAULT_MEDIA_PIPELINE,
