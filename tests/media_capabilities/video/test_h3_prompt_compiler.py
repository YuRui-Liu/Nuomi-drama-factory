...
def test_compiles_complete_i2va_in_deterministic_official_wire_order():
    ...
    assert compile_h3_director_plan(plan) == (
        ...
        "integrated_multimodal_description: Visual style: cinematic realism.\n"
        "Continuity locks: same face and black coat; same iron door and lighting.\n"
        "[Shot 1] Frames 0-101 (0.00-4.21s). medium close-up; eye level; "
        "focus on Lin Mo; composition: Lin Mo remains centered against the iron door. "
        "Camera: slow, subtle push-in moving forward.\n"
        "Action establish, frames 0-12 (0.00-0.50s): Lin Mo braces against the door.\n"
        "Action execute, frames 12-101 (0.50-4.21s): He turns toward the rattling handle.\n"
        "Lin Mo (S1) says at frames 48-84 (2.00-3.50s): "
        "<d>[Chinese]别过来</d>\n\n"
...
def test_multi_shot_dialogue_keeps_stable_speaker_and_continuation_markers():
    first = _shot(
        end_frame=50,
        dialogue=(
            H3DialogueCue(
    ...
    assert "Lin Mo (S1) says at frames 20-45" in prompt
    assert "<d>[Chinese]别开门——</d> [truncated]" in prompt
    assert "Lin Mo (S1) says at frames 55-90" in prompt
    assert "<d>[Chinese]外面不是人。</d> [continuation]" in prompt
