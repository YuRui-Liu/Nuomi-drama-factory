...
@pytest.mark.asyncio
async def test_native_ollama_embedding_engine_is_limited_without_provider_attribute():
    from novelvideo.cognee.concurrency import (
        CogneeConcurrencyConfig,
        build_limited_index_data_points,
        cognee_pipeline_concurrency,
    ...
    async def scheduler(_points, vector_engine=None):
        await asyncio.gather(
            *(vector_engine.index_data_points(index) for index in range(8))
        )

    limited = build_limited_index_data_points(scheduler, OllamaEmbeddingEngine)
    async with cognee_pipeline_concurrency(
        CogneeConcurrencyConfig(llm=1, embedding_batch=2)
    ):
        await limited([], vector_engine=VectorEngine())
...
def test_init_cognee_does_not_swallow_concurrency_install_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _settings: ("ollama", "embedding", "1024", ""),
    )
