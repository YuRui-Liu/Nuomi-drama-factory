...
5. Show the user the normalized style, six fragment groups, provenance, preview prompt, and three target files. **Stop and wait for explicit approval.** Preparing a plan is never approval to generate or write.
...
```bash
python -m pytest skills/publishing-nuomi-extension-styles/tests/test_publish_style.py \
