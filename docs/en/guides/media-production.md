...
The output path must be absolute, its real (non-symlink) parent must already exist, and neither the target nor its adjacent `<output>.receipt.json` may exist; the gate never overwrites previous evidence. Before a billable call it executes `ffprobe -version`, so a missing or unusable probe blocks submission. Every invocation uses a new temporary runtime ledger and reports `runtime_cache=fresh`, so a persistent cache hit cannot count as release evidence. As soon as RunningHub accepts the submission, its callback writes the task ID, fixed workflow ID, and input digest atomically to the receipt and also prints the task ID to stderr. The receipt deliberately contains no credentials and survives later polling, download, probing, or size failures.

After the runtime returns, the gate independently requires a readable non-empty artifact, uses `ffprobe` to require the exact selected H3 dimensions, and prints the output SHA-256 and actual size before `status=succeeded`. The temporary ledger is then discarded without touching application runtime data.
